...
if ($result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc()) {
        ...
        if(substr(($row["PurchaseDate"]),3)==$Now){
        	echo '<tr style="background: rgba(255,221,0,0.1);"><td>$'.$TotalPurchaseThousands.'</td><td>'.$OdometerThousands.'</td><td>'.$row["FuelType"].'</td><td>'.$FuelPriceThousands.'</td><td>'.$row["FuelAmount"].'</td><td>'.$row["PurchaseDate"].' '.$row["PurchaseTime"].'</td><td>'.$row["Place"].'</td><td><a href="#" title="Editar Registro" data-toggle="modal" data-target="#add-fuel-modal" onclick="updateFuelEntryModal(`'.$row["PK_Id"].'`,`'.$TotalPurchaseThousands.'`, `'.$OdometerThousands.'`, `'.$row["FuelType"].'`, `'.$FuelPriceThousands.'`, `'.$row["FuelAmount"].'`, `'.$row["PurchaseDate"].'`, `'.$row["PurchaseTime"].'`, `'.$row["Place"].'`);"><i class="fa fa-pencil text-info" style="margin:0 5px;"></i></a><a href="#" title="Eliminar Registro" data-toggle="modal" data-target="#fuelmore-modal" onclick="deleteFuelEntryModal(`'.$row["PK_Id"].'`,`'.$TotalPurchaseThousands.'`, `'.$OdometerThousands.'`, `'.$row["FuelType"].'`, `'.$FuelPriceThousands.'`, `'.$row["FuelAmount"].'`, `'.$row["PurchaseDate"].'`, `'.$row["PurchaseTime"].'`, `'.$row["Place"].'`);"><i class="fa fa-trash text-danger" style="margin:0 5px;"></i></a></td></tr>';
        }else{
